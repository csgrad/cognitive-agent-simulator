...
ylim([-50, 2100])
h1 = errorbar(p1,avgdata(:,1),stddata(:,1),'g','LineWidth',2);		% errorbars
h1Children = get(h1, 'children');
set(h1Children(2), 'linestyle','--')
plot(p1,avgdata(:,1),'-g.','LineWidth',3,'MarkerSize',24)			% lines between averages
h2 = errorbar(p2,avgdata(:,2),stddata(:,2),'r','LineWidth',2);
h2Children = get(h2, 'children');
set(h2Children(2), 'linestyle','--')
plot(p2,avgdata(:,2),'-r.','LineWidth',3,'MarkerSize',24)
h3 = errorbar(p3,avgdata(:,3),stddata(:,3),'b','LineWidth',2);
h3Children = get(h3, 'children');
set(h3Children(2), 'linestyle','--')
plot(p3,avgdata(:,3),'-b.','LineWidth',3,'MarkerSize',24)
h = [h1;h2];
h = [h;h3];
...
print(gcf,'-djpeg99','-loose','errorbar-skq');
